    ...
    useEffect(() => {
        async function getStats() {
            ...
            if (response && response.status === 200) {
                ...
                let completed = response.data.GamesCompleted
                setGamesCompleted(Object.keys(completed).map(game => {
                    return {name: game, value: completed[game]}
                }))
        ...
        getStats()
    }, [setGamesPlayed, setTotalPlayed, setGamesCompleted, setTotalCompleted, setActiveGames, setTotalActiveGames, setActivePlayers, setTotalActivePlayers])
